...
export default {
  ...
  methods: {
    exportToExcel() { // On Click Excel download button
      ...
      me.$axios({
        method: 'get',
        url: 'person-export', /* enviamos la url de la api y la ruta con sus parametros para descargar el csv */
        /* responseType: 'blob' */
      })
      .then(res => {
        if (res.data.length) {
          ...
          XLSX.utils.book_append_sheet(wb, exportData, 'Personas') // sheetAName is name of Worksheet
          // export Excel file
          XLSX.writeFile(wb, 'Personas.xlsx') // name of the file is 'book.xlsx'
          me.$swal({
