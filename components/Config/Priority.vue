...
export default {
  ...
  methods: {
    exportToExcel() { // On Click Excel download button
      ...
      me.$axios({
        method: 'get',
        url: 'priority-export', /* enviamos la url de la api y la ruta con sus parametros para descargar el csv */
        /* responseType: 'blob' */
      })
      .then(res => {
        if (res.data.length) {
          ...
          XLSX.utils.book_append_sheet(wb, exportData, 'Prioridades') // sheetAName is name of Worksheet
          // export Excel file
          XLSX.writeFile(wb, 'Prioridades.xlsx') // name of the file is 'book.xlsx'
          me.$swal({
