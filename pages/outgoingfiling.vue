<template>
  ...
  <el-card class="box-card outgoing_filing">
    <div
      slot="header"
      class="clearfix">
    ...
    <div>
      <div>
        <b-button
          v-permission="'create_based_out'"
          variant="primary"
          @click="newOutGoingFiling(false)"><i class="fas fa-plus-circle"/> Nuevo</b-button>
        ...
        <b-button
          target="_blank"
          variant="warning"
          @click.prevent="showPDFTemplate">
          <i class="fas fa-clipboard-list"/> Generar planilla
        </b-button>
      </div>
      <div
        class="mt-2 pt-3 body_outgoing">
        <template>
          <div class="container-fluid overflow-auto">
            ...
            <!-- Pintar Tabla -->
            <b-table
              v-if="outgoingFiling.length"
              id="table-outgoingFiling"
              :fields="fields"
              :items="outgoingFiling"
              :per-page="perPage"
              :filter="filter"
            ...
            <b-pagination
              ...
              :total-rows="rows"
              :per-page="perPage"
              aria-controls="table-outgoingFiling"
            />
          </div>
        </template>
      </div>
      ...
      <!-- info modal anular -->
      <b-modal
        ref="modal-outgoingFiling-cancel"
...
export default {
  ...
  created() {
    ...
    let showPage =
      storePermissions.find(item => item === 'see_based_out') === undefined
    if (showPage) {
      this.$router.push('/')
    }else {
  ...
  methods: {
    exportToExcel() { // On Click Excel download button
      ...
      me.$axios({
        method: 'get',
        url: `outgoingfiling/export?fromDate=${fromDate}&toDate=${toDate}`, /* enviamos la url de la api y la ruta con sus parametros para descargar el csv */
        /* responseType: 'blob' */
      })
      .then(res => {
        if (res.data.length) {
          ...
          XLSX.utils.book_append_sheet(wb, exportData, 'RadicacionSalida') // sheetAName is name of Worksheet
          // export Excel file
          XLSX.writeFile(wb, 'RadicacionSalida.xlsx') // name of the file is 'book.xlsx'
          me.$swal({
    ...
    modalCancel(item, index, button) {
      ...
      this.$refs['modal-outgoingFiling-cancel'].show()
    },
    ...
    hideModalCancel() {
      this.$refs['modal-outgoingFiling-cancel'].hide()
      setTimeout(() => {
        this.modalValidate = null
        this.cancelFiling = {
          description: null,
    ...
    showPDFTemplate() {
      if (!this.dateRange.length) {
        window.open(process.env.apiBaseUrl + `/template-outgoing-filing`)
      } else {
        window.open(
          process.env.apiBaseUrl +
            `/template-outgoing-filing?fromDate=${this.dateRange[0]}&toDate=${
              this.dateRange[1]
            }`
        )
      }
